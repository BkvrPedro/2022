...
const { listarEmprestimos } = require('./emprestimos');
...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const livroEmprestado = await conexao.query('select * from emprestimos where id_livro = $1', [id]);

        if (livroEmprestado.rowCount > 0) {
            return res.status(400).json('Livro emprestado, não é possível excluir.');
        }
